...
│   */trace_*.json    │                               │  · incremental scan, TTL     │
...
| POST   | `/config/poll`        | Sync refresh interval — sets the backend trace-scan TTL globally |
| POST   | `/shutdown`           | Flush and gracefully exit the backend process        |
...
- `token-widget/src-tauri/target/release/bundle/nsis/token-widget_0.2.3_x64-setup.exe` — NSIS installer
- `token-widget/src-tauri/target/release/bundle/msi/token-widget_0.2.3_x64_en-US.msi` — MSI installer
